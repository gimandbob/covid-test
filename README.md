...
기능적 요구사항
1. 보건소가 코로나 검사를 한다.
1. 보건소가 검사 키트를 진단소에 전송한다.
1. 검사 키트가 보내지면 진단소는 진단 요청을 받는다.
1. 진단소는 결과를 진단한다.
1. 보건소는 진단 결과를 조회할 수 있다.
1. 보건소는 잘못된 검사를 취소할 수 있다.
1. 검사가 취소되면 진단도 취소된다.

비기능적 요구사항
1. 트랜잭션
    1. 검사 키트를 진단소에 전송해야 전체 검사 과정이 완료된다. (동기화) 
1. 장애격리
    1. 진단 기능이 수행되지 않아도 검사는 365일 24시간 받을 수 있어야 한다. Async (event-driven), Eventual Consistency
    1. 검사 키트 전송이 많이 밀려있다면 잠시후에 전송할 수 있게 유도한다. Circuit breaker, fallback
1. 성능
    1. 보건소는 진단 결과를 확인 할 수 있어야 한다. CQRS
